...
var SUPPRESS_NOTIFICATION = false; // Number of records to get for each page.
// Warning: this must be the same as the page size set in the NetSuite connector.

var PAGE_SIZE = 10; // Field name for subscriptions.

var subscriptionsFieldName = 'subscriptions'; // GET function.
...
function getRecord(datain) {
  ...
  var page = 1;
  if (datain.page != null) page = datain.page;
  var filters = null;
  var columns = null;
...
function buildFilters(datain) {
  var filters = []; // Temporary record used for getting field types.
...
function getRecordFieldValue(record, fieldName, fieldValue) {
  if (!record || !fieldName) return fieldValue;
...
function getFieldValue(field, fieldValue) {
  if (!field) return fieldValue; // Convert ISO date to account date.

  if (field.type === 'date') return nlapiDateToString(new Date(fieldValue + 'T08:00:00.000Z'), 'date'); // Convert ISO date time to JS Date.

  if (field.type === 'datetime' || field.type === 'datetimetz') return new Date(fieldValue); // Convert checkbox boolean to T/F

  if (field.type === 'checkbox') return fieldValue ? 'T' : 'F';
  return fieldValue;
...
function transformRecord(record) {
  ...
  var transformed = JSON.parse(JSON.stringify(record)); // Transform fields.
  ...
  for (var i = 0; i < lineItems.length; i++) {
    ...
    if (!Array.isArray(transformedLineItem) || transformedLineItem.length < 1) continue;
...
function setRecord(record, datain) {
  ...
  for (var fieldName in datain) {
    if (!datain.hasOwnProperty(fieldName) || fieldName === 'recordtype' || fieldName === 'id') continue;
...
function createSublists(record, sublistName, sublistFields) {
  if (!Array.isArray(sublistFields)) sublistFields = [sublistFields];
  ...
  for (var sublistField in sublistFields) {
    ...
    if (!field) {
      ...
      try {
        ...
        throw nlapiCreateError('CYCLR_INVALID_SUBLIST_FIELD', 'Sublist name: ' + sublistName + '\tField name: ' + sublistField, SUPPRESS_NOTIFICATION);
      }
    ...
    if (field.type === 'select') {
      if (sublistValue.internalid != null) record.setCurrentLineItemValue(sublistName, sublistField, sublistValue.internalid);else if (sublistValue.name != null) record.setCurrentLineItemText(sublistName, sublistField, sublistValue.name);
      continue;
    }

    if (field.type === 'multiselect') {
      if (!Array.isArray(sublistValue)) sublistValue = [sublistValue];
      if (sublistValue.length < 1) continue;
...
function setRecordFieldValue(record, fieldName, fieldValue) {
  ...
  if (!field) {
    throw nlapiCreateError('CYCLR_INVALID_FIELD', 'Field name: ' + fieldName, SUPPRESS_NOTIFICATION);
  }

  if (field.type === 'select') {
    if (fieldValue.internalid != null) record.setFieldValue(fieldName, fieldValue.internalid);else if (fieldValue.name != null) record.setFieldText(fieldName, fieldValue.name);
    return;
  }
...
function setSubrecordValues(subrecord, sublistValue) {
  if (_typeof(sublistValue) === 'object') {
...
function setSubscriptions(record, datain) {
  ...
  if (count == 0) return; // No subscriptions to set.
  // Get the subscription internal ids and names.

  var subscriptionIdNameMap = JSON.parse(JSON.stringify(record))[subscriptionsFieldName].reduce(function (accumulator, currentValue) {
  ...
  for (var i = 1; i <= count; i++) {
    ...
    if (!subscriptionIdNameMap.hasOwnProperty(subscriptionId)) continue; // Find the subscription name.

    var subscriptionName = subscriptionIdNameMap[subscriptionId]; // Find the datain that matches the subscription.
    ...
    for (var c = 0; c < datain.subscriptions.length; c++) {
      ...
      if (datain.subscriptions[c].subscription.internalid != null && datain.subscriptions[c].subscription.internalid == subscriptionId) {
        datainSubscription = datain.subscriptions[c];
        break;
      } else if (datain.subscriptions[c].subscription.name != null && datain.subscriptions[c].subscription.name === subscriptionName) {
        datainSubscription = datain.subscriptions[c];
        break;
      }
    }
    ...
    var valueToSet = null;
    if (datainSubscription == null) valueToSet = 'F'; // Subscription not provided in update.
    else valueToSet = datainSubscription.subscribed ? 'T' : 'F';
